...
static LPWSTR BuildPath(LPCWSTR Dir, LPCSTR File, UINT Att)
{
    UINT cp = Att & _A_NAME_IS_UTF ? CP_UTF8 : CP_ACP;
...
static HRESULT HResultFrom(const ERF &erf)
{
    ...
    {
        case FDIERROR_NONE: return erf.fError ? HRESULT_FROM_WIN32(erf.erfType) : S_OK;
        case FDIERROR_CABINET_NOT_FOUND: return HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND);
        case FDIERROR_ALLOC_FAIL: return E_OUTOFMEMORY;
        case FDIERROR_USER_ABORT: return S_FALSE;
        default: return erf.erfType ? HRESULT_FROM_WIN32(erf.erfType) : E_FAIL;
    }
}
...
static INT_PTR CabOpenEx(LPCWSTR path, UINT access, UINT share, UINT disp, UINT att)
{
    return (INT_PTR)CreateFileW(path, access, share, NULL, disp, att, NULL);
}
...
{
    hfdi = FDICreate(CabMemAlloc, CabMemFree, CabOpen, CabRead, CabWrite, CabClose, CabSeek, cpuUNKNOWN, &erf);
        ...
        case fdintCOPY_FILE:
            ...
            if (hr == S_OK)
            {
                ...
                if (path)
                {
                    ...
                    //    file[-1] = L'\\';
                    //}
                    UINT att = pfdin->attribs & (FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_ARCHIVE);
                    INT_PTR handle = CabOpenEx(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_DELETE, CREATE_NEW, att | FILE_FLAG_SEQUENTIAL_SCAN);
                    noti.hr = HRESULT_FROM_WIN32(GetLastError());
                    SHFree(path);
                    if (handle != (INT_PTR)-1)
                        return handle;
...
HRESULT ExtractCabinet(LPCWSTR cab, LPCWSTR destination, EXTRACTCALLBACK callback, LPVOID cookie)
{
    ...
    INT_PTR hf = quick ? -1 : CabOpenEx(cab, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_DELETE, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL);
    if (hf != -1)
    {
        FDICABINETINFO ci;
