...
static inline WORD Swap16(WORD v) { return MAKEWORD(HIBYTE(v), LOBYTE(v)); }
static inline UINT Swap32(UINT v) { return MAKELONG(Swap16(HIWORD(v)), Swap16(LOWORD(v))); }
#define BigToHost32 Swap32

template<class T> static inline ULARGE_INTEGER MakeULargeInteger(T value)
{
    ULARGE_INTEGER ret;
    ret.QuadPart = value;
    return ret;
...
{
    if (size > 8 + (8 + (4 + 4 + 5) + 4) + (8 + 4) && IsPngSignature(file))
    {
        const UINT* chunkhdr = (UINT*)((char*)file + 8);
        if (BigToHost32(chunkhdr[0]) >= (4 + 4 + 5) && chunkhdr[1] == 0x52444849)
        {
...
static void OverrideFileContent(HGLOBAL& hMem, DWORD& Size)
{
    ...
    {
        const UINT minbmp = sizeof(BITMAPCOREHEADER), minpng = 8 + (8 + (4 + 4 + 5) + 4);
...
#else
                // Place the best image first, GDI+ (WIC in ROS) will return the first image
...
static HRESULT LoadImageFromFileHandle(HANDLE hFile, GpImage** ppImage)
{
    ...
    if (buffer)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                pStream->SetSize(MakeULargeInteger(size)); // CreateStreamOnHGlobal does not know the real size
