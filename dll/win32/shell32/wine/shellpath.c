...
HRESULT SHGetFolderLocationHelper(HWND hwnd, int nFolder, REFCLSID clsid, LPITEMIDLIST *ppidl)
{
    ...
    EXTERN_C HRESULT SHELL32_BindToObject(IShellFolder *psf, LPCITEMIDLIST pidl, REFIID riid, void **ppvObj);
    *ppidl = NULL;
    if (FAILED(hr = SHGetFolderLocation(hwnd, nFolder, NULL, 0, &parent)))
        return hr;
    if (SUCCEEDED(hr = SHELL32_BindToObject(NULL, parent, &IID_IShellFolder, (void**)&psf)))
    {
