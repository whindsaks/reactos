...
#ifdef __REACTOS__
    /* This functions name is misleading, it only knows about FS items */
    return lpPData && (lpPData->type & (PT_BASEMASK | PT_FS_UNICODE)) == PT_FS_UNICODE;
#else
...
static LPWSTR _ILGetTextPointerW(LPCITEMIDLIST pidl)
{
    ...
    case PT_FS_DIRW:
    case PT_FS_FILEW:
        /* TODO: Check _ILGetFileStructW(pidl) first? */
        return (LPWSTR)pdata->u.file.szNames;
#endif
...
#ifdef __REACTOS__
/*************************************************************************
* _ILItemGetFSItemType
*
* Returns SHDID_FS_FILE, SHDID_FS_DIRECTORY or SHDID_FS_OTHER for CFSFolder items, 0 for other items.
*/
DWORD _ILItemGetFSItemType(PCUITEMID_CHILD pidl)
{
    enum { minsize = sizeof(WORD) + 1 + FIELD_OFFSET(FileStruct, szNames) + 1 + 1 };
    if (_ILGetBaseType(pidl) == PT_FS && pidl->mkid.cb >= minsize)
    {
        BYTE fileordir = pidl->mkid.abID[0] & (PT_FS_DIR | PT_FS_FILE);
        FileStruct *pfs = (FileStruct*)(&pidl->mkid.abID[1]);
        C_ASSERT(SHDID_FS_DIRECTORY == ((PT_FS_DIR - PT_FS - 1) ^ 3));
        C_ASSERT(SHDID_FS_FILE == ((PT_FS_FILE - PT_FS - 1) ^ 3));
        if (fileordir < (PT_FS_DIR | PT_FS_FILE))
            return (fileordir - PT_FS - 1) ^ 3;

        /* PIDLs that should not exist; simple with non-zero data but we might as well check */
        if (pfs->uFileAttribs & FILE_ATTRIBUTE_DIRECTORY)
            return SHDID_FS_DIRECTORY;
        if ((pfs->dwFileSize | pfs->uFileAttribs) != 0)
            return SHDID_FS_FILE;

        return SHDID_FS_OTHER; // Generated by SHSimpleIDList, don't know if it is a file or a folder
    }
    return 0;
}
#endif
