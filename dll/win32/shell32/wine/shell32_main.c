                                ...
                                SHFILEINFOW *psfi, UINT sizeofpsfi, UINT flags )
{
    ...
    if (!(flags & SHGFI_PIDL))
    {
        ...
        if (PathIsRelativeW(path))
        {
            ...
            path = szFullPath;
        }OutputDebugStringW(path);OutputDebugStringW(L"<SHGFI\n");
    ...
    if (SUCCEEDED(hr) && (flags & (SHGFI_ICON | SHGFI_SYSICONINDEX)))
    {
        ...
        if (flags & SHGFI_ICON)
        {
            ...
            // TODO: SHELL_GetItemAttributes(psfParent, pidlLast, SFGAO_LINK)
            psfi->hIcon = ImageList_GetIcon(hSIL, psfi->iIcon, uILFlags);
            if (psfi->hIcon && !(flags & SHGFI_SHELLICONSIZE))
            {
                INT w = GetSystemMetrics((flags & SHGFI_SMALLICON) ? SM_CXSMICON : SM_CXICON);
                INT h = GetSystemMetrics((flags & SHGFI_SMALLICON) ? SM_CYSMICON : SM_CYICON);
                psfi->hIcon = (HICON)CopyImage(psfi->hIcon, IMAGE_ICON, w, h, LR_COPYDELETEORG | LR_COPYRETURNORG);
            }
        }
    }
