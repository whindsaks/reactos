...
SFGAOF SHELL_FSAttributesToSFAttributes(DWORD FS)
{
    SFGAOF Out = (FS & FILE_ATTRIBUTE_DIRECTORY) ? SFGAO_FOLDER : SFGAO_STREAM;
    if (FS & FILE_ATTRIBUTE_SYSTEM) Out |= SFGAO_SYSTEM;
    if (FS & FILE_ATTRIBUTE_HIDDEN) Out |= SFGAO_HIDDEN;
    if (FS & FILE_ATTRIBUTE_READONLY) Out |= SFGAO_READONLY;
    if (FS & FILE_ATTRIBUTE_ENCRYPTED) Out |= SFGAO_ENCRYPTED;
    if (FS & FILE_ATTRIBUTE_COMPRESSED) Out |= SFGAO_COMPRESSED;
    return Out;
}
