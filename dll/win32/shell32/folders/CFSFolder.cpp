...
getIconLocationForFolder(IShellFolder * psf, PCITEMID_CHILD pidl, UINT GilIn,
                         LPWSTR szIconFile, UINT cchMax, int *piIndex, BOOL AllowFallback = TRUE)
{
    DWORD dwFileAttrs;
    ...
    if (!AllowFallback && (GilIn & (GIL_FORSHORTCUT | GIL_OPENICON)))
        goto Quit; // IDefaultExtractIconInit callers can skip desktop.ini for non-Normal
...
HRESULT CFSExtractIcon_CreateInstance(IShellFolder * psf, LPCITEMIDLIST pidl, REFIID iid, LPVOID * ppvOut)
{
    ...
    *szIco = UNICODE_NULL;
    ...
    if (_ILIsFolder (pidl))
    {
        ...
        // if (SUCCEEDED(getIconLocationForFolder(psf, pidl, GIL_FORSHORTCUT, szIco, _countof(szIco), &icon_idx, FALSE)))
        //     initIcon->SetShortcutIcon(szIco, icon_idx);
        if (SUCCEEDED(getIconLocationForFolder(psf, pidl, GIL_OPENICON, szIco, _countof(szIco), &icon_idx, FALSE)))
    ...
    else
    {
        ...
        if (hkey && !SHQueryValueExW(hkey, NULL, NULL, NULL, szIco, &dwSize))
        {
            ...
            if (!wcscmp(L"%1", szIco) || !wcscmp(L"\"%1\"", szIco)) /* icon is in the file */
            {
                bPercentOne = TRUE;
        ...
        if (!*szIco)
        {
            ...
            UINT Flags = bPercentOne ? GIL_PERINSTANCE : 0;
            icon_idx = SHELL_GetShell32IconLocation(Flags, bPercentOne ? pExtension : NULL, szIco);
        }
        initIcon->SetNormalIcon(szIco, icon_idx);
