...
using namespace CFSFolderItemFormat;
static HRESULT SHELL32_GetCLSIDForDirectory(LPCWSTR pwszDir, LPCWSTR KeyName, CLSID* pclsidFolder);

UINT CFSFolder::Item::GetExtraSize() const
{
    PCUITEMID_CHILD pidl = (PCUITEMID_CHILD)this;
    // Validation compatible with WINE "XP" PIDLs
    WORD cbOffset = *(WORD*)((BYTE*)pidl + pidl->mkid.cb - sizeof(WORD));
    ITEMEXTRA_WXP *pX = (ITEMEXTRA_WXP*)((BYTE*)pidl + cbOffset);
    const UINT cbmin95 = 2 + 1 + FIELD_OFFSET(FileStruct, szNames[1 + 1]);
    if (!(cbOffset & 0x1) && cbOffset >= cbmin95 && // Word aligned extra data starts after the filenames
        cbOffset < pidl->mkid.cb - sizeof(cbOffset) - sizeof(ITEMEXTRA_WXP) && // Extra data must fit at the end
        pidl->mkid.cb == cbOffset + pX->cb)
    {
        if (pX->NameOffset)
            return pX->NameOffset;
        return sizeof(ITEMEXTRA_WXP); // WINE is broken, assume its a FileStructW
    }
    return 0;
}

LPCWSTR CFSFolder::Item::GetFSName(LPWSTR Buffer, UINT MaxBuf) const
{
#if 0
    // FIXME: Replace this with correct handling of all FS pidl types
    LPCITEMIDLIST self = (LPCITEMIDLIST)(PCUITEMID_CHILD)this;
    FileStructW *psw = _ILGetFileStructW(self);
    if (psw && psw->wszName[0])
        return psw->wszName;
    DWORD len = _ILSimpleGetTextW(self, Buffer, MaxBuf);
    if (len)
        return Buffer;
    if ((abID[0] & PT_FOLDERW) == PT_FOLDERW) // pidl.c fails to handle this
        return (LPWSTR)InfoW95().szNames;
    return NULL;
#else
    if (ITEMEXTRA_WXP *pX = Extra<ITEMEXTRA_WXP>())
    {
        return (LPCWSTR)((BYTE*)pX + pX->NameOffset);
    }
    if (IsUnicodeItemType())
        return (LPCWSTR)InfoW95().szNames;
    SHAnsiToUnicode(InfoW95().szNames, Buffer, MaxBuf);
    return Buffer;
#endif
}

void CFSFolder::Item::GetFSNameToBuffer(LPWSTR Buffer, UINT MaxBuf) const
{
    LPCWSTR strptr = GetFSName(Buffer, MaxBuf);
    if (strptr != Buffer)
        StrCpyNW(Buffer, strptr ? strptr : L"", MaxBuf);
}

DWORD CFSFolder::Item::GetSHDID() const
{
    BYTE fileordir = abID[0] & (PT_FS_DIR | PT_FS_FILE);
    ...
    if (GetFSAttributes() & FILE_ATTRIBUTE_DIRECTORY)
        return SHDID_FS_DIRECTORY;
    if ((InfoW95().dwFileSize | GetFSAttributes()) != 0)
        return SHDID_FS_FILE;
    return SHDID_FS_OTHER; // Generated by old SHSimpleIDList, don't know if it is a file or a folder
}

CFSFolder::Item* CFSFolder::Validate(PCUITEMID_CHILD pidl) const
{
    if (pidl && pidl->mkid.cb > 2 + 1 + FIELD_OFFSET(FileStruct, szNames) && _ILFastGetBaseType(pidl) == PT_FS)
        return (CFSFolder::Item*)pidl;
    return NULL;
}

HRESULT CFSFolder::GetItemSize(const Item &ItemId, UINT64 &Size)
{
    LARGE_INTEGER li;
    li.LowPart = ItemId.InfoW95().dwFileSize;
    if (ITEMEXTRA_WVISTA *pX = ItemId.Extra<ITEMEXTRA_WVISTA>())
    {
        li.HighPart = pX->SizeHi;
        Size = li.QuadPart;
        return S_OK;
    }
    li.HighPart = 0;
    ...
    STRRET str;
    HRESULT hr = GetDisplayNameOf(ItemId, SHGDN_FORPARSING, &str);
    if (SUCCEEDED(hr))
    {
        CComHeapPtr<WCHAR> path;
        if (SUCCEEDED(hr = StrRetToStrW(&str, (PCUITEMID_CHILD)&ItemId, &path)))
        {
            HANDLE hFile = CreateFileW(path, FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, NULL);
...
HRESULT CFSFolder::GetItemFindData(const Item &ItemId, WIN32_FIND_DATAW &wfd)
{
    ZeroMemory(&wfd, sizeof(wfd));
    UINT cbX = ItemId.GetExtraSize();
    UINT64 size;
    HRESULT hr = GetItemSize(ItemId, size);
    if (SUCCEEDED(hr))
    {
        wfd.nFileSizeLow = (UINT)size;
        wfd.nFileSizeHigh = (UINT)(size << 32);
    }
    wfd.dwFileAttributes = ItemId.GetFSAttributes();
    DosDateTimeToFileTime(ItemId.InfoW95().uFileDate, ItemId.InfoW95().uFileTime, &wfd.ftLastWriteTime);
    if (cbX >= sizeof(ITEMEXTRA_WXP))
    {
        ITEMEXTRA_WXP &xp = ItemId.FastExtra<ITEMEXTRA_WXP>();
        DosDateTimeToFileTime(xp.CDate, xp.CTime, &wfd.ftCreationTime);
        DosDateTimeToFileTime(xp.ADate, xp.ATime, &wfd.ftLastAccessTime);
    }
    ItemId.GetFSNameToBuffer(wfd.cFileName, MAX_PATH);
    // The short name is tricky
    LPCSTR names = ItemId.InfoW95().szNames;
    bool unicode = ItemId.IsUnicodeItemType();
    UINT len = unicode ? lstrlenW((LPWSTR)names) : lstrlenA(names);
    if (len < 14)
        ItemId.GetFSNameToBuffer(wfd.cAlternateFileName, 14);
    else
        SHAnsiToUnicode(&names[(len + 1) * (unicode + 1)], wfd.cAlternateFileName, 14);
    return hr;
}
...
// Note: scidmap and GenericSFHeader must be in the same order
static const SHCOLUMNID* scidmap[] = {
    &PKEY_ItemNameDisplay, // SHFSF_COL_NAME
    &PKEY_Size, // SHFSF_COL_SIZE
    &PKEY_ItemTypeText, // SHFSF_COL_TYPE
    &PKEY_DateModified, // SHFSF_COL_MDATE
    &PKEY_FileAttributes, // SHFSF_COL_FATTS
    NULL, // SHFSF_COL_COMMENT
};
        ...
        PCUITEMID_CHILD_ARRAY apidl, DWORD * rgfInOut)
{
    ...
    if(cidl == 0)
    {
        ...
        {
            DbgPrint("HACK: CFSFolder accessing item as a drive\n");
...
int CFSFolder::MapSCIDToColumn(const SHCOLUMNID &scid)
{
    C_ASSERT(_countof(GenericSFHeader) == _countof(scidmap));
    for (UINT i = 0; i < _countof(scidmap); ++i)
    {
        if (scidmap[i] && IsEqualPropertyKey(*scidmap[i], scid))
            return i;
    }
    return -1;
}
...
{
    if (column < _countof(scidmap) && scidmap[column])
    {
        *pscid = *scidmap[column];
        return S_OK;
    }
    return E_INVALIDARG;
}
                                       ...
                                       const SHCOLUMNID * pscid, VARIANT * pv)
{
    ...
    int col;
    Item *pItem = Validate(pidl);

    if (!pItem || !pscid)
    {
        return E_INVALIDARG;
    }
    else if ((col = MapSCIDToColumn(*pscid)) >= 0)
    {
        switch(col)
        {
            case SHFSF_COL_SIZE:
            {
                UINT64 size;
                if (SUCCEEDED(hr = GetItemSize(*pItem, size)))
                {
                    V_VT(pv) = VT_UI8;
                    V_UI8(pv) = size;
                }
            ...
            {
                hr = E_FAIL;
                if (DosDateTimeToVariantTime(pItem->InfoW95().uFileDate, pItem->InfoW95().uFileTime, &V_DATE(pv)))
                {
                    V_VT(pv) = VT_DATE;
                    hr = S_OK;
                }
            ...
            {
                V_VT(pv) = VT_UI4;
                V_UI4(pv) = pItem->GetFSAttributes();
                break;
            }
            default: // String columns don't need special handling
            {
                SHELLDETAILS sd;
                if (SUCCEEDED(hr = GetDetailsOf(pidl, col, &sd)))
                    hr = SHELL_InitVariantFromStrRet(&sd.str, pidl, pv);
            }
        }
    }
    else if (IsEqualPropertyKey(*pscid, PKEY_FindData))
    {
        ...
        if (SUCCEEDED(hr = GetItemFindData(*pItem, wfd)))
            return SHELL_InitVariantFromBuffer(&wfd, sizeof(wfd), pv);
    }
    else if (IsEqualPropertyKey(*pscid, PKEY_DescriptionID))
    {
        SHDESCRIPTIONID shdid;
        shdid.dwDescriptionId = pItem->GetSHDID();
        shdid.clsid = GUID_NULL; // FIXME (TODO: GetItemCLSID)
        return SHELL_InitVariantFromBuffer(&shdid, sizeof(shdid), pv);
    }
    return hr;
}
