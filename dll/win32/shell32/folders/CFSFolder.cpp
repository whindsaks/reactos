...
{
    BYTE type = _ILGetType(pidl), both = PT_FS | PT_FS_FOLDER_FLAG | PT_FS_FILE_FLAG;
    return (type & both) == (PT_FS | PT_FS_FOLDER_FLAG) || (type == PT_FS && ILGetNext(pidl));
}
...
static LPCWSTR GetItemFileName(PCUITEMID_CHILD pidl, LPWSTR Buf, UINT cchMax)
{
    ...
    if (pdata && (pdata->type & PT_VALUEW) == PT_VALUEW) // TODO: || PT_FOLDERW
